...
export default class extends React.Component {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      choroPlethData: null,
      choroPleathPopUpdIndicator: "Creation of surgical airway",
      currentViewType: "choropleth"
    };
  }

  componentDidMount () {
      let {countiesData,geoJson,categories}= insertSurveyValuesToGeoJson(6,10343, MapData,52,"Creation of surgical airway");
  ...
  handleCategoryChange = (categoryId)=>{
    if(categoryId==null) categoryId=52; //default
    let categoryName="";
    this.state.categories.forEach((categoryArry)=>{
      categoryArry.forEach((categry)=>{
        if(categry.id==categoryId){
          categoryName=categry.name;
        }
      });
    });
    let {countiesData,geoJson,categories}=
        insertSurveyValuesToGeoJson(6,10343, MapData,categoryId,
                categoryName,this.state.surveyData);
    this.setState({
      choroPlethData: geoJson,
      choroPleathPopUpdIndicator: categoryName
    });
  }
    ...
    return (
        <React.Fragment>

          {
            ...
            <p style={{textAlign: 'center', fontWeight: 'bold'}}>Percentage of facilities that offer emergency airway interventions</p>
            <div id="emegencyAirwayMapId"  style={{minWidth: 200 + 'px', width: 800 + 'px', minHeight: 200 + 'px', height: 700 + 'px', position: 'relative'}}>
                <div className="catgeory-select">
                    <CategoryFilter  handleCategoryChange={this.handleCategoryChange} catList={this.state.categories}/>
                </div>

                <LeafletMap ref="emegencyAirwayMap" scrollWheelZoom={false} center={center} zoomControl={false} minZoom={6.4} zoom={6.4} maxZoom={6.4} />

                <SurveyChoroplethLegend elId={'surveyChroplethLegend'}  grades ={ [0, 4, 6, 8, 10, 12, 14] } getChoroplethColor={this.getChoroplethColor}/>
            </div>
