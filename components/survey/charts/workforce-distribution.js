...
export default class extends React.Component {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      choroPlethData: null,
      choroPleathPopUpdIndicator: "Core health workforce per 10,000 population",
      currentViewType: "choropleth"
    };
  }

  componentDidMount () {
      let {countiesData,geoJson,categories}=   insertSurveyValuesToGeoJson(6,10334, MapData,null,"Core health workforce per 10,000 population");
  ...
  componentDidUpdate(prevProps, prevState) {
      if(prevState.choroPlethData != this.state.choroPlethData ){
        let geoJsonLayer=L.geoJSON(this.state.choroPlethData, {
          ...
          } ).addTo(this.refs.workforceDistributionMap.leafletElement);
    ...
    return (
        <React.Fragment>

          {
            ...
            <p style={{textAlign: 'center', fontWeight: 'bold'}}>Core health workforce per 10,000 population</p>
            <div id="workforceDistributionMap"  style={{minWidth: 200 + 'px', width: 800 + 'px', minHeight: 200 + 'px', height: 700 + 'px', position: 'relative'}}>

                <LeafletMap ref="workforceDistributionMap" scrollWheelZoom={false} center={center} zoomControl={false} minZoom={6.4} zoom={6.4} maxZoom={6.4} />

                <SurveyChoroplethLegend elId={'workforceDistributionChroplethLegend'} grades ={ [0, 4, 8, 12, 16, 20, 24 ] } getChoroplethColor={this.getChoroplethColor}/>
            </div>
