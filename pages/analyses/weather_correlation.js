...
import { fetchIndicators, FetchCountyList, FetchWeatherIndicatorCorrelation, getIndicatorScatterDataArray } from '../../components/utils/Helpers'
...
export default class extends React.Component{
  ...
  onSelectIndic=(selectedList, selectedItem)=> {
    ...
    this.fetchCorrData(selectedItem.id,this.state.ouid);
    this.setState({
      id: selectedItem.id
    });
  }
  ...
  onRemoveIndic=(selectedList, removedItem)=> {
    ...
    if(step!=0)
      this.fetchCorrData(this.state.id,this.state.ouid);
      this.setState({
        id: indicList
      });
  }
  ...
  onSelectOrgunit=(selectedList, selectedItem)=> {
    ...
    this.fetchCorrData(this.state.id,selectedItem.id);
  }
  ...
  componentDidMount(){
    this.getIndicators();
    this.getCounties();
    this.fetchCorrData(this.props.query.id,this.props.query.ouid);

  }

  fetchCorrData=(id,ouid)=>{
    (async () => { //http://dsl.health.go.ke/dsl/api/pandemics/covid19?id=6074&start_date=2020-06-07
      let {indicatorData}=await FetchWeatherIndicatorCorrelation(id,ouid);
      const correData = [];
      try{
      ...
      let scatterData = []
      for(let key in indicatorData.result.data.weather){
        let scatterList=getIndicatorScatterDataArray(indicatorData.result.data.weather[key], indicatorData.result.data.indicator[this.state.id]);
        let scatterMap = {}
  ...
  render() {
    ...
    if(this.state.scatterData){
      ...
      for(let key in this.state.correlationData.result.dictionary.weather){
        weatherMap[key]=this.state.correlationData.result.dictionary.weather[key];
      }
                ...
                <div className="columns">
                  ...
                  <div className= "column bcwhite br-5">
                        ...
                        <div className="container">
                            ...
                            <h4 className="title is-5 text-center text-uppercase fcsecondary-dark text-bold">Weather correlation analyses</h4>
